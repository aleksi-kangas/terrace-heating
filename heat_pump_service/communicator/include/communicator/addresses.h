...
struct BoostingScheduleAddresses {
  ...
  constexpr BoostingScheduleAddresses(WeekdayAddresses monday, WeekdayAddresses tuesday, WeekdayAddresses wednesday,
                                      WeekdayAddresses thursday, WeekdayAddresses friday, WeekdayAddresses saturday,
  ...
  [[nodiscard]] constexpr int32_t FirstHour() const {
    return std::min({monday.start_hour, monday.end_hour, tuesday.start_hour, tuesday.end_hour, wednesday.start_hour,
                     wednesday.end_hour, thursday.start_hour, thursday.end_hour, friday.start_hour, friday.end_hour,
                     saturday.start_hour, saturday.end_hour, sunday.start_hour, sunday.end_hour});
  }

  [[nodiscard]] constexpr int32_t LastHour() const {
    return std::max({monday.start_hour, monday.end_hour, tuesday.start_hour, tuesday.end_hour, wednesday.start_hour,
                     wednesday.end_hour, thursday.start_hour, thursday.end_hour, friday.start_hour, friday.end_hour,
                     saturday.start_hour, saturday.end_hour, sunday.start_hour, sunday.end_hour});
  }

  [[nodiscard]] constexpr int32_t QuantityHour() const { return LastHour() - FirstHour() + 1; }

  [[nodiscard]] constexpr int32_t FirstDelta() const {
    return std::min(
        {monday.delta, tuesday.delta, wednesday.delta, thursday.delta, friday.delta, saturday.delta, sunday.delta});
  }
  ...
  [[nodiscard]] constexpr int32_t QuantityDelta() const { return LastDelta() - FirstDelta() + 1; }
};

constexpr BoostingScheduleAddresses kCircuit3{{5214, 5213, 107}, {5211, 5212, 106}, {5220, 5221, 110},
                                              {5222, 5215, 111}, {5223, 5224, 112}, {5216, 5217, 108},
                                              {5218, 5219, 109}};
constexpr BoostingScheduleAddresses kLowerTank{{5014, 5021, 36}, {5015, 5022, 37}, {5016, 5023, 38}, {5017, 5024, 39},
