...
std::shared_ptr<grpc::ServerCredentials> MakeSslServerCredentials(
    const std::filesystem::path& server_private_key_file,
  ...
  grpc::SslServerCredentialsOptions::PemKeyCertPair pem_key_cert_pair{
      .private_key = server_private_key, .cert_chain = server_certificate};
  grpc::SslServerCredentialsOptions ssl_server_credential_options{};
  ssl_server_credential_options.client_certificate_request =
      GRPC_SSL_REQUEST_AND_REQUIRE_CLIENT_CERTIFICATE_AND_VERIFY;
  ssl_server_credential_options.force_client_auth = true;
  ssl_server_credential_options.pem_key_cert_pairs = {
      std::move(pem_key_cert_pair)};
...
int main() {
  ...
  const int32_t kHeatPumpPort = []() {
    const std::string kHeatPumpPortStr =
        ReadEnvironmentVariable("HEAT_PUMP_PORT");
    if (kHeatPumpPortStr.empty()) {
      throw std::runtime_error{
          "HEAT_PUMP_PORT environment variable is not set"};
    }
    try {
      return std::stoi(kHeatPumpPortStr);
    } catch (const std::invalid_argument&) {
      throw std::runtime_error{
          "HEAT_PUMP_PORT environment variable is not a valid port"};
    }
  }();
  ...
  auto server_credentials =
      MakeSslServerCredentials("/usr/src/app/certs/heat-pump-service-key.pem",
                               "/usr/src/app/certs/heat-pump-service-cert.pem",
                               "/usr/src/app/certs/ca-cert.pem");
  ...
  service::HeatPumpService service{
      std::make_unique<communicator::ModbusTCPCommunicator>(kHeatPumpIp,
                                                            kHeatPumpPort)};
